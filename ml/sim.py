...
def _cap_stakes(
    ...
    if stake_cap_market is not None:
        bets = (
            bets
            .with_columns(pl.col("stake_unit").cum_sum().over("marketId").alias("cum_stake_mkt"))
            .with_columns(
    ...
    if stake_cap_day is not None:
        bets = (
            bets
            .with_columns(pl.col("stake_unit").cum_sum().over("event_date").alias("cum_stake_day"))
            .with_columns(
...
def main():
    ...
    ap.add_argument("--top-n-per-market", type=int, default=1, help="Select top N by EV in each market")
    ap.add_argument("--stake-cap-market", type=float, default=1.0, help="Max total stake units per market")
    ap.add_argument("--stake-cap-day", type=float, default=10.0, help="Max total stake units per day")
