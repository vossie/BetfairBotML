...
# ml/sim2.py — streaming simulator with back/lay guardrails, EV gate, partial fills, persistence, and schema-safe concat
...
def _select_feature_cols(df: pl.DataFrame, label_col: str) -> List[str]:
    exclude = {"sport","marketId","selectionId","ts","ts_ms","publishTimeMs",label_col,"runnerStatus","p_hat"}
    cols: List[str] = []
...
def _edge_columns(df: pl.DataFrame, side: str = "auto") -> pl.DataFrame:
    ...
    out = df.with_columns([pl.when(pl.col(src) > 1.0).then(pl.col(src)).otherwise(None).alias("odds")]).drop_nulls(["odds"])
...
def _cap_stakes(bets: pl.DataFrame, cap_market: float, cap_day: float, min_floor: float) -> pl.DataFrame:
    ...
    b1 = (
        ...
        .with_columns([
            pl.when(pl.col("_sum_mkt") > 0).then((pl.lit(cap_market) / pl.col("_sum_mkt"))).otherwise(pl.lit(0.0)).alias("_mkt_scale")
        ])
    ...
    out = b5.rename({"_stake_grouped": "stake"}).drop("_priority_edge","_priority_stake_unit","_stake_mkt","_day_bucket")
    return out
...
# -------------------- core simulate --------------------
def simulate_once(args, df_feat: pl.DataFrame, booster_single: Optional[xgb.Booster], booster_30: Optional[xgb.Booster], booster_180: Optional[xgb.Booster]) -> Dict[str, float]:
    ...
    state = StreamState(args.cooldown_secs, args.max_open_per_market, args.max_exposure_day, args.latency_ms)
    ...
    def _try_fill_unmatched(arrivals_df: pl.DataFrame, now_ms: int):
        ...
        for od in state.unmatched:
            if now_ms - od["placed_ms"] > args.rest_secs * 1000 or args.persistence == "lapse":
                stream_log_rows.append({"publishTimeMs": now_ms, "marketId": od["marketId"], "selectionId": od["selectionId"], "event": "lapse_unfilled", "remaining": od["remaining"]})
                continue
            ...
            if fill_amt <= 0:
                still.append(od); continue
            exec_rows.append({
                "marketId": od["marketId"], "selectionId": od["selectionId"], "publishTimeMs": now_ms,
                "tto_minutes": row.get("tto_minutes"), "side": od["side"], "stake": float(fill_amt),
                "odds": row.get("odds", best_px), "odds_exec": float(best_px),
                "winLabel": row.get("winLabel"), "p_hat": row.get("p_hat"), "edge": row.get("edge"),
            })
            stream_log_rows.append({"publishTimeMs": now_ms, "marketId": od["marketId"], "selectionId": od["selectionId"],
        ...
        if exec_rows:
            placed_rows.append(_exec_rows_df(exec_rows))
    ...
    for _, arrivals in df.group_by("_bucket", maintain_order=True):
        ...
        for r in cands.iter_rows(named=True):
            ...
            if side == "lay":
                mult = max(px_exec - 1.0, 1e-9)
                max_stake_by_liab = args.max_liability_per_bet / mult
                if max_stake_by_liab <= 0:
                    stream_log_rows.append({"publishTimeMs": now_ms, "marketId": mkt, "selectionId": sel, "event": "reject", "reason": "liability_cap_zero"})
                    continue
        ...
        if executed_rows:
            placed_rows.append(_exec_rows_df(executed_rows))

    # Build bets df even if empty (schema-safe)
    if placed_rows:
...
# -------------------- CLI --------------------
def main():
    ap = argparse.ArgumentParser(description="Streaming-style wagering sim (back & lay) with guardrails.")
    # Mode
    ...
    # Execution realism
    ap.add_argument("--min-stake", type=float, default=1.0, help="Minimum tradable stake (Betfair min ~ £1).")
