cd /opt/BetfairBotML
cat > ml/sim2_all.py <<'PY'
# ml/sim2_all.py — load-once + score-once + parameter sweep (best combo by ROI)
...
# Reuse core pieces from sim2.py (already in your repo)
from .sim2 import (
    simulate_once,        # single simulation run on a prebuilt dataframe
...
def _parse_list_or_range(val: str, example: Any) -> List[Any]:
    ...
    if val.startswith("[") and val.endswith("]"):
        ...
        if ":" in inner and inner.count(":") in (1,2):
            parts = inner.split(":")
            if len(parts)==2:
                start, stop = float(parts[0]), float(parts[1]); step = (stop - start)
            else:
...
def main():
    ...
    ap.add_argument("--model-30", default=None)
    ap.add_argument("--model-180", default=None)
    ...
    ap.add_argument("--sweep-grid", required=True,
                    help='Grid ranges/lists, e.g. side=[back,lay,auto], min_edge=[0.06:0.12:0.02], kelly=[0.05,0.1,0.25]')
    ap.add_argument("--sweep-parallel", type=int, default=0, help="0=cpu_count, 1=serial, N=workers")
    ap.add_argument("--sweep-write-bets", action="store_true", help="Write bets per combo (slower)")
    ...
    # score once and reduce to decision columns
    if single:
    ...
    # write once for workers to mmap (fast, low RAM per proc)
    ...
    if workers == 1:
        ...
        with Pool(processes=workers) as pool:
            for row in pool.imap_unordered(_run_combo, payloads):
                results.append(row)
