...
def add_features_with_future(
    ...
    steps = max(1, int(round(horizon_secs / 5)))  # 5s snapshots
    ...
    # Apply window (keep only rows with a future value)
...
def level1_fill_ok(df: pl.DataFrame, min_fill_frac: float) -> pl.Series:
    """
    Very simple liquidity gate:
    - If backSizes / laySizes present, approximate level-1 back size >= min_fill_frac
      (we treat min_fill_frac as a nominal size in £ for simplicity).
    - If not present, pass-through (assume ok).
    """
    have = set(df.columns)
    need = {"backSizes","laySizes"}
    if not need.issubset(have):
        return pl.Series([True] * df.height, dtype=pl.Boolean)
    # Use first level size from list
...
def kelly_fraction_back_from_ev(ev_per_1: float, odds: float, commission: float) -> float:
    """
    Proxy p from EV & odds:
        EV ≈ p*(o-1) - (1-p) = p*o - 1  =>  p ≈ (EV + 1)/o
    Kelly back:
        f* = (b*p - q) / b,  with b = net_back_odds(o), q = 1 - p
    """
...
def size_stake(
    ...
    if stake_mode == "flat":
        # equal split of market budget across selected slots
        unit = per_market_budget / max(1, slots_in_market)
        return max(0.0, float(unit))
    # kelly proxy
    f = kelly_fraction_back_from_ev(ev, odds, commission)
    f = min(kelly_cap, max(kelly_floor, f))
    stake = bankroll * f
    # cap by per-market budget quota
...
def predict_dp(booster, feats_df: pl.DataFrame, feature_names: list[str], batch_size: int = 200_000) -> pl.Series:
    """
    Predict dp_target using XGB model. We select intersection of available columns with model's expected names.
    """
...
def select_trades(
    ...
    out = df
    # Edge & odds
    out = out.filter(pl.col("ev_per_1").is_not_null() & (pl.col("ev_per_1") >= float(edge_thresh)))
...
def compute_pnl(
    ...
    # Expected PnL proxy: EV per £1 * stake
    trades = trades.with_columns((pl.col("ev_per_1") * pl.col("stake")).alias("exp_pnl"))

    # Realised (MTM) using computed future delta __dp
    # If exit_on_move_ticks>0, we can approximate by clamping the realised move to +/- exit ticks
    if exit_on_move_ticks and "ltp_lag30s" in trades.columns:
        # Use tick size ~ odds granularity: assume 0.01 (Betfair varies, but OK as a proxy)
        tick = 0.01
    ...
    # Settlement proxy = MTM (we note this clearly in summary)
...
def parse_args():
    ...
    ap.add_argument("--device", choices=["cuda","cpu"], default="cpu")  # placeholder (CPU inference by default)
    ap.add_argument("--batch-size", type=int, default=200_000)

    # Trading flags (compat with scripts)
    ...
    ap.add_argument("--liquidity-levels", type=int, default=1)  # placeholder
    ap.add_argument("--min-fill-frac", type=float, default=0.25)
    ap.add_argument("--per-market-topk", type=int, default=1)
    ap.add_argument("--per-market-budget", type=float, default=10.0)  # £ per market
...
def main():
    ...
    base_cols = [
        ...
        # If liquidity is requested, try to project these too (when present)
    ...
    for day in days:
        ...
        # Join to defs to bring marketStartMs (+ country)
        ...
        # Predict dp_target with model when available; else fall back to observed __dp (this makes it "oracle EV")
        if booster is not None and model_feature_names:
            # pick model features if present; else naive set (drop identifiers)
            id_drop = {"marketId","selectionId","publishTimeMs","marketStartMs","countryCode","ltp_future","__dp"}
            feats_cols = [c for c in df.columns if c not in id_drop]
            try:
                pred = predict_dp(booster, df.select(feats_cols), model_feature_names, batch_size=args.batch_size)
        ...
        # Persist per-day EV rows (for debugging / downstream)
        out_parquet = out_dir / f"sim_{day}.parquet"
        try:
            df.write_parquet(out_parquet)
        except Exception:
            pass  # non-fatal

    # Aggregate trades across days
    if not trades_rows:
        log("[simulate] No trades selected across days.")
        # Still write a minimal summary for sweeps to consume
        summary = {
            "edge_thresh": args.edge_thresh,
            "stake_mode": args.stake_mode,
            "odds_min": args.odds_min,
    ...
    # Persist summary JSON (fields expected by sweep/automl)
    summary = {
        "edge_thresh": args.edge_thresh,
        "stake_mode": args.stake_mode,
        "odds_min": args.odds_min,
        ...
        # Settlement not joined; proxy equals MTM (documented)
