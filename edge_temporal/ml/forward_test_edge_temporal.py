...
"""
Forward tester:
- Loads model.json, isotonic.pkl, and best_config.json from a model dir
- Applies the saved policy (pm_cutoff, edge_thresh, ltp band) with pre-off <= N minutes (default 180)
- Computes per-day ROI table over an arbitrary date range
- Flat staking by default; Kelly optional
"""
...
def main():
    ...
    ap.add_argument("--model-dir", required=True, help="Directory containing model.json, isotonic.pkl, best_config.json")
    ap.add_argument("--preoff-max", type=int, default=180)
    ap.add_argument("--commission", type=float, default=None, help="Override commission; default = read from best_config")
    ...
    ap.add_argument("--bankroll-nom", type=float, default=None, help="Override bankroll; default = read from best_config")
    ...
    y=y_all[use_mask]
    base = (
        df.select(["marketId","publishTimeMs","selectionId","ltp"])
    ...
    for d in uniq_days:
        ...
        df_day=base.filter(pl.Series(day_mask))
        met=evaluate(
            df_day,
