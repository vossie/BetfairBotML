#!/usr/bin/env python3
# Polars 1.33 compatible. CUDA by default. Env-driven backtest filters.
# Optional: --fit-calib (OOF isotonic) and/or load CALIB_PATH env to apply calibration.
...
def evaluate(df_valid, odds, y, p_model, p_market, edge_thresh, topk, lo, hi,
    ...
    df = df.with_columns(pl.col("edge").rank(method="dense", descending=True).over("marketId").alias("rk")).filter(pl.col("rk") <= topk).drop("rk")
...
def parse_args():
    ...
    # calibration
    p.add_argument("--fit-calib", action="store_true", help="Fit OOF isotonic on TRAIN and apply to VALID; also saves to --calib-out if provided.")
...
def main():
    ...
    # optional: fit calibrator on TRAIN and apply to VALID
    iso = None
    if args.fit_calib:
    ...
    for d in sorted(days):
        ...
        if iso is not None: pv = iso.predict(pv).astype(np.float32)
